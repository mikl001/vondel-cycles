...
/** Slugs are generated by slugify() — lowercase, digits and hyphens only.
 *  Validating here keeps user-controlled [slug] segments out of the PostgREST
 *  filter grammar entirely (no comma/paren/dot injection into the .or() string). */
...
export async function getProductReviews(
  ...
  return safe({ reviews: [], summary: { count: 0, average: 0 } }, async () => {
    ...
    // count + average in a single indexed query (no second full-table scan)
    const { data: summaryRows, error: err2 } = await supabase.rpc(
    ...
    return {
      ...
      summary: {
        count: Number(summaryRow?.review_count ?? count ?? 0),
